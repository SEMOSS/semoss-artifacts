...
if ! [[ -z "${SEMOSS_VERSION}" ]] || [[ (( $last_updated > $updated )) ]]; then
        ...
        cd /opt/semoss-artifacts/artifacts/home && mvn clean install -Dci.version=$version -q
	...
	rm -r /opt/semoss-artifacts/artifacts/home/target
        cd /opt/semoss-artifacts/artifacts/web && mvn clean install -Dci.version=$version -q
	...
	rm -r /opt/semoss-artifacts/artifacts/web/target
        cd /opt/semoss-artifacts/artifacts/war && mvn clean install -Dci.version=$version -q
	...
	rm -r /opt/semoss-artifacts/artifacts/war/target
        cd /opt/semoss-artifacts/artifacts/lib && mvn clean install -Dci.version=$version -q
