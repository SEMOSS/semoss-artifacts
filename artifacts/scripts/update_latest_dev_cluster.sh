...
latest_version=`curl -L -s "https://oss.sonatype.org/content/repositories/public/org/semoss/semosscluster/maven-metadata.xml" | grep "<latest>.*</latest>" | sed -e "s#\(.*\)\(<latest>\)\(.*\)\(</latest>\)\(.*\)#\3#g"`
last_updated=`curl -L -s "https://oss.sonatype.org/content/repositories/public/org/semoss/semosscluster/maven-metadata.xml" | grep "<lastUpdated>.*</lastUpdated>" | sed -e "s#\(.*\)\(<lastUpdated>\)\(.*\)\(</lastUpdated>\)\(.*\)#\3#g"`
