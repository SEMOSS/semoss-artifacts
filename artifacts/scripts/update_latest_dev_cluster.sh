...
latest_version=`curl -s "https://oss.sonatype.org/content/repositories/snapshots/org/semoss/semosscluster/maven-metadata.xml" | grep "<latest>.*</latest>" | sed -e "s#\(.*\)\(<latest>\)\(.*\)\(</latest>\)\(.*\)#\3#g"`
last_updated=`curl -s "https://oss.sonatype.org/content/repositories/snapshots/org/semoss/semosscluster/maven-metadata.xml" | grep "<lastUpdated>.*</lastUpdated>" | sed -e "s#\(.*\)\(<lastUpdated>\)\(.*\)\(</lastUpdated>\)\(.*\)#\3#g"`
...
if ! [[ -z "${SEMOSS_VERSION}" ]] || [[ (( $last_updated > $updated )) ]]; then
        ...
        cp /opt/semoss-artifacts/artifacts/cluster/semosscluster-$version.jar /opt/semosscluster-$version.jar
